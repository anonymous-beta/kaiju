...
def zone_transfer(domain: str) -> str:
    ...
    try:
        ...
        return f"ERROR: no nameservers resolved: {e}"

    out = [f"Zone transfer attempt: {domain}"]
    for ns in ns_answers:
        ns_host = str(ns).rstrip(".")
        try:
            # Resolve the NS to an IP to query directly
            try:
                ip = str(res.resolve(ns_host, "A")[0])
            except Exception:
                ip = ns_host
            z = dns.zone.from_xfr(dns.query.xfr(ip, domain, timeout=8, lifetime=15))
            names = sorted(z.keys())
            out.append(f"  [+] SUCCESS via {ns_host} ({ip}) — {len(names)} records")
            for n in names[:200]:
                out.append(f"      {n} → {z[n]}")
        except Exception as e:
            out.append(f"  [-] {ns_host}: transfer refused ({e})")
    return "\n".join(out)
...
TOOLS = [
    Tool("dns_lookup", "Resolve DNS records (A, AAAA, MX, TXT, NS, CNAME, SOA) for a domain",
         dns_lookup, category="recon"),
    Tool("subdomain_enum", "Brute-force subdomains via DNS with a built-in wordlist or custom file",
         subdomain_enum, category="recon"),
    Tool("whois_lookup", "WHOIS record: registrar, dates, nameservers, contacts",
         whois_lookup, category="recon"),
    Tool("osint_lookup", "Combined WHOIS + DNS + IP OSINT snapshot of a target",
         osint_lookup, category="recon"),
    Tool("reverse_dns", "PTR lookup: IP → hostname",
         reverse_dns, category="recon"),
    Tool("zone_transfer", "Attempt DNS zone transfer (AXFR) against all nameservers",
         zone_transfer, category="recon"),
  ]
